...
function spawnInherited(args, label) {
  console.log(`\n[dev] Starting ${label}...`);
  const proc = spawn(NODE, args, { stdio: 'inherit', cwd: ROOT_DIR });
  children.push(proc);
  proc.on('error', (err) => {
...
function freePort(port) {
  try {
    if (process.platform === 'win32') {
      const cmd = `powershell -NoProfile -Command "(Get-NetTCPConnection -LocalPort ${port} -ErrorAction SilentlyContinue).OwningProcess"`;
...
async function main() {
  // Clean up any stale processes holding port 8080 or 3000
  freePort(8080);
  freePort(3000);
  ...
  spawnInherited([VITE_BIN], 'client (vite)');
}
