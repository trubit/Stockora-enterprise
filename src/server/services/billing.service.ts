...
export class BillingService {
  ...
  public static async initializeSubscriptionPayment(input: InitializeSubscriptionPaymentInput) {
    ...
    // Statutory Nigerian VAT is 7.5% (0.075) for all NGN and Paystack transactions
    let configuredRate =
      typeof tenant?.taxConfig?.defaultTaxRate === 'number'
        ? tenant.taxConfig.defaultTaxRate
        : 0.075;

    // Handle percentage stored as whole number e.g. 7.5 -> 0.075
    if (configuredRate > 1) {
      configuredRate = configuredRate / 100;
    }

    // For NGN and Paystack subscriptions, statutory VAT of 7.5% is mandatory
    const taxRate =
  ...
  public static async ensureDefaultPlans(): Promise<void> {
    const freePlanData = {
      ...
      slug: 'free',
      tier: 'FREE' as const,
      description: 'Basic single-register point of sale for micro-businesses.',
      price: 0,
      currency: 'NGN',
    ...
    if (count > 0) {
      const existingFree = await Plan.findOne({ tier: 'FREE' });
      if (!existingFree) {
        await Plan.create(freePlanData);
        logger.info('[Billing] Initialized default FREE tier SaaS subscription plan.');
      }
      return;
    }
    ...
    const defaultPlans = [
      freePlanData,
      {
        ...
        slug: 'starter',
        tier: 'STARTER',
        description: 'Ideal for single-location retail stores and growing boutiques.',
        price: 25000,
        currency: 'NGN',
      ...
      },
      {
        ...
        slug: 'professional',
        tier: 'PROFESSIONAL',
        description: 'Multi-branch operations with centralized inventory and staff controls.',
        price: 65000,
        currency: 'NGN',
        billingInterval: 'MONTHLY',
        yearlyDiscountPercent: 20,
      ...
      },
      {
        ...
        slug: 'enterprise',
        tier: 'ENTERPRISE',
        description: 'Unlimited warehouse logistics, real-time sync, and API keys.',
        price: 150000,
        currency: 'NGN',
        billingInterval: 'MONTHLY',
    ...
    logger.info('[Billing] Initialized default SaaS subscription plans.');
  }
}
